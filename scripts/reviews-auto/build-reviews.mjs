...
async function main() {
  ...
  //
  // This must cover the LOCALIZED homepages too, not just the root. They are
  // generated by scripts/i18n-build.mjs and the reviews pipeline never touched
  // them, so they froze at whatever count was current when they were last
  // regenerated — live check on 2026-07-26 showed the root correctly at 15
  // while /fr/ /de/ /pt/ /es/ /it/ all still claimed 10. Every one of those
  // pages declares the SAME business @id, so publishing five different review
  // counts for one entity is exactly the kind of contradiction that gets a
  // rich result dropped.
  const homepages = [
    INDEX_HTML,
    ...["fr", "de", "pt", "es", "it"].map((l) => join(SITE_ROOT, l, "index.html")),
  ];
  let schemaUpdated = 0;
  for (const file of homepages) {
    ...
    if (after !== before) { writeFileSync(file, after); schemaUpdated++; }
  }
  if (schemaUpdated) {
    console.log(`[build] updated aggregateRating on ${schemaUpdated} homepage(s) to ${aggregate.rating.toFixed(1)}/${aggregate.count}`);
  }
